...
public class ActionGroupSAXWriter  {

 /** 
  * Writes an array of TG objects to xml 
  *
 **/
 public static void write (List<com.stottlerhenke.simbionic.common.xmlConverters.model.Action> dmObjects, PrintWriter writer, int indent) {
    if (dmObjects == null) return; 
    for (Iterator it = dmObjects.iterator(); it.hasNext(); ) {
      com.stottlerhenke.simbionic.common.xmlConverters.model.Action dmChild = (com.stottlerhenke.simbionic.common.xmlConverters.model.Action)it.next();
      Utils.writeStartTag("action",writer,indent+1);
      ActionSAXWriter.write(dmChild,writer,indent+2);
      Utils.writeEndTag("action",writer,indent+1);
    }
 }
